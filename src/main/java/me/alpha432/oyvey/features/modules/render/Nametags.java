...
public class Nametags extends Module
{
    ...
    public Nametags() {
        ...
        this.pops = (Setting<Boolean>)this.register(new Setting("Pop Count", true));
    ...
    public void renderNametag(final EntityPlayer player, final double x, final double y, final double z) {
        ...
        final boolean isFriend = OyVey.friendManager.isFriend(player.getName()) && this.friends.getValue();
        ...
        PopCounter.getInstance();
        String string;
        if (PopCounter.TotemPopContainer.get(player.getName()) != null && this.pops.getValue()) {
            final StringBuilder append2 = new StringBuilder().append(" ").append(ChatFormatting.DARK_RED).append("-");
            PopCounter.getInstance();
            string = append2.append(PopCounter.TotemPopContainer.get(player.getName())).toString();
        }
        else {
            string = "";
        }
        String name = append.append(string).toString();
        name = name.replace(".0", "");
        final EntityPlayerSP entityPlayerSP = (EntityPlayerSP)((Nametags.mc.getRenderViewEntity() == null) ? Nametags.mc.player : Nametags.mc.getRenderViewEntity());
        ...
        if (FontMod.getInstance().isOn()) {
            this.customFont.drawStringWithShadow(name, (double)(-width), 8.649999618530273, -1);
        }
        else {
    ...
    public void renderItem(final EntityPlayer player, final ItemStack stack, final int x, final int y, final int nameX, final boolean showHeldItemText) {
        ...
        if (!this.shownItem && this.item.getValue() && showHeldItemText) {
            if (FontMod.getInstance().isOn()) {
                this.customFont.drawString(stack.getDisplayName().equalsIgnoreCase("Air") ? "" : stack.getDisplayName(), (double)(nameX * 2 + 95 - this.customFont.getStringWidth(stack.getDisplayName()) / 2), (double)(y - 37), -1, true);
            }
            else {
    ...
    private void renderDurabilityText(final EntityPlayer player, final ItemStack stack, final int x, final int y) {
        ...
        if (stack.getItem() instanceof ItemArmor || stack.getItem() instanceof ItemSword || stack.getItem() instanceof ItemTool) {
            ...
            if (FontMod.getInstance().isOn()) {
                this.customFont.drawStringWithShadow(dmg + "%", (double)(x * 2 + 4), (double)(y - 10), ColourHolder.toHex((int)(red * 255.0f), (int)(green * 255.0f), 0));
            }
            else {
    ...
    public void renderEnchantText(final EntityPlayer player, final ItemStack stack, final int x, final int y) {
        ...
        if (this.enchantMode.getValue() == EnchantMode.MAX && this.isMaxEnchants(stack)) {
            ...
            if (FontMod.getInstance().isOn()) {
                this.customFont.drawStringWithShadow("Max", (double)(x * 2 + 7), (double)(yCount + 24), 16711680);
            }
            else {
                ...
                for (int index = 0; index < enchants.tagCount(); ++index) {
                    ...
                    if (enc != null && !enc.isCurse()) {
                        ...
                        if (encName.contains("Pr") || encName.contains("Bl")) {
                            ...
                            GL11.glScalef(1.0f, 1.0f, 0.0f);
                            if (FontMod.getInstance().isOn()) {
                                this.customFont.drawStringWithShadow(encName, (double)(x * 2 + 3), (double)yCount, -1);
                            }
                            else {
